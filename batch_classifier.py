...
def do_for_one_batch(batch_dois, batch_texts, all_file_results):
    ###################################################################################
    (distil_pooled_vecs, distil_token_vecs, distil_masks) = distilbert_vectorizer.emit_for_abstracts(batch_texts)
    (bert_pooled_vecs, _, _) = bert_vectorizer.emit_for_abstracts(batch_texts)
    ###################################################################################
    ...
    #################################
    start_time = time.perf_counter()
    r1 = k1_1.emit_for_abstracts(batch_texts, distil_pooled_vecs)
    print("Elapsed time 2: ", time.perf_counter() - start_time)
    #################################
    start_time = time.perf_counter()
    r2 = k1_2.emit_for_abstracts(batch_texts, distil_pooled_vecs)
    print("Elapsed time 3: ", time.perf_counter() - start_time)
    #################################
    start_time = time.perf_counter()
    r3 = k1_3.emit_for_abstracts(batch_texts, bert_pooled_vecs)
    print("Elapsed time 4: ", time.perf_counter() - start_time)
    #################################
    start_time = time.perf_counter()
    bert_results, bert_results_att = k4.emit_for_abstracts(batch_texts,distil_pooled_vecs,distil_token_vecs,distil_masks)
    print("Elapsed time 5: ", time.perf_counter() - start_time)
    #################################
    start_time = time.perf_counter()
    guided_sdg_res = glda.emit_for_abstracts(batch_texts)
    print("Elapsed time 6: ", time.perf_counter() - start_time)
    ###################################################################################
